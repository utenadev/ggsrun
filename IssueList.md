...
### 現状の問題点
大規模なリファクタリングの途中で、ビルドが通らない状態になっていた。複数のファイルで、パッケージのインポート不足、不要なインポート、関数の二重定義、未定義関数の呼び出し、構文エラーなどが混在していた。

### 提案する解決策
リファクタリング作業を再開する前に、まずコードベースを安定させ、ビルドが成功するように修正する。

### 具体的なタスクリスト
- [x] `go build` で報告されるすべてのコンパイルエラーを修正する。
    - [x] 未使用のパッケージインポートを削除する。
    - [x] 不足しているパッケージをインポートする。
    - [x] 重複して定義されていた定数や関数を削除する。
    - [x] リファクタリング途中で未定義となっていた関数に対し、一時的なプレースホルダー（仮実装）を追加する。
    - [x] 構文エラー（文字列リテラルの終端など）を修正する。
    - [x] 非推奨の関数 (`ioutil.ReadFile`) を新しい関数 (`os.ReadFile`) に置き換える。
